  ...
  try {
    ...
    if (updates.hasOwnProperty("isActive")) {
      return res.status(400).json({
        status: 400,
        msg: "The 'isActive' field cannot be updated through this API. Use the separate API for that.",
      });
    }
...
export const updatePackageIsActiveStatus = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: 200,
      msg: `Package ${isActive ? "activated" : "deactivated"} successfully`,
      package: updatedPackage,
    });
  } catch (error) {
    return res.status(500).json({
